  ...
  useEffect(() => {
    ...
    function animate() {
      ...
      for (let i = pointsRef.current.length - 1; i >= 0; i--) {
        ...
        const startColor = getColor(point.colorOffset + timeRef.current * 0.0005);
        const midColor1 = getColor(point.colorOffset + 0.33 + timeRef.current * 0.0005);
        const midColor2 = getColor(point.colorOffset + 0.66 + timeRef.current * 0.0005);
